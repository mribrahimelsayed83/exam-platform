...
router.put('/students/:id', staff, async (req, res) => {
  const { name, grade, phone, parent_phone } = req.body;
  try {
    await pool.query(
      `UPDATE students SET name=$1, grade=$2, phone=$3, parent_phone=$4 WHERE id=$5`,
      [name, grade, phone, parent_phone, req.params.id]
    );
